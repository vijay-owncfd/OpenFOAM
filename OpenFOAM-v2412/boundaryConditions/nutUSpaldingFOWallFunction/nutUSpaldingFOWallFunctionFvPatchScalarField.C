...
#include "nutUSpaldingFOWallFunctionFvPatchScalarField.H"
...
Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::calcNut() const
{
...
Foam::tmp<Foam::scalarField>
Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::calcUTau
(
...
) const
{
    ...
    const fvPatchVectorField& Uw = U(turbModel).boundaryField()[patchi];
    vectorField Up(Uw.patchInternalField() - Uw);   // relative velocity w.r.t. wall
    Up -= n*(n & Up);                               // tangential component of relative velocity w.r.t. wall
        ...
        if (ROOTVSMALL < ut)
        {
            ...
            do
            {
                ...
                const scalar f =
                    ...
                    + magUp[facei]/ut
                    + 1.0/E*(fkUu - 1.0/6.0*pow(kUu, 3) - 1.0/24.0*pow(kUu, 4));

                const scalar df =
                    y[facei]/nuw[facei]
                  + magUp[facei]/sqr(ut)
                  + 1.0/E*(kUu/ut*(fkUu - 1.0/6.0*pow(kUu, 3)));
...
void Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::writeLocalEntries
(
    Ostream& os
) const
{
...
Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::
nutUSpaldingFOWallFunctionFvPatchScalarField
(
    const fvPatch& p,
...
nutUSpaldingFOWallFunctionFvPatchScalarField
(
    const nutUSpaldingFOWallFunctionFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
nutUSpaldingFOWallFunctionFvPatchScalarField
(
    const nutUSpaldingFOWallFunctionFvPatchScalarField& wfpsf
)
:
...
~nutUSpaldingFOWallFunctionFvPatchScalarField()
{
    //if (debug)
    //{
    //    Info<< "nutUSpaldingFOWallFunctionFvPatchScalarField :"
    //        << " total invocations:"
...
Foam::tmp<Foam::scalarField>
Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::yPlus() const
{
    const label patchi = patch().index();
...
void Foam::nutUSpaldingFOWallFunctionFvPatchScalarField::write
(
    Ostream& os
) const
{
